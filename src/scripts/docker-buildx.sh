...
TAG=${!PARAM_TAG}
SKIP_WHEN_TAGS_EXIST=${!PARAM_SKIP_WHEN_TAGS_EXIST}
REPO=${!PARAM_REPO}
EXTRA_BUILD_ARGS=${!PARAM_EXTRA_BUILD_ARGS}
FILE_PATH=${!PARAM_PATH}
DOCKERFILE=${!PARAM_DOCKERFILE}
PROFILE_NAME=${!PARAM_PROFILE_NAME}
REGISTRY_ID=${!PARAM_REGISTRY_ID}
REGION=${!!AWS_REGION}
PLATFORM=${!PARAM_PLATFORM}
PUBLIC_REGISTRY=${!PARAM_PUBLIC_REGISTRY}
PUSH_IMAGE=${!PARAM_PUSH_IMAGE}
...
for tag in "${DOCKER_TAGS[@]}"; do
  if [ "${SKIP_WHEN_TAGS_EXIST}" = "1" ]; then
      docker_tag_exists_in_ecr=$(aws "${ECR_COMMAND}" describe-images --profile "${PROFILE_NAME}" --registry-id "${REGISTRY_ID}" --region "${REGION}" --repository-name "${REPO}" --query "contains(imageDetails[].imageTags[], '${tag}')")
